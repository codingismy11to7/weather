...
import weather.util.OWMClient.{ClientMethods, OneCallResponse}
...
object WeatherService {
  ...
  private def responseToResult(response: OneCallResponse) = {
    val base = WeatherResult(tempToType(response.current.temp))
    val withCondition =
      response.current.weather.headOption
        .map(w => base.copy(condition = w.main, conditionStr = w.description))
        .getOrElse(base)
    withCondition.copy(alerts =
      response.alerts.getOrElse(Nil).map(a => WeatherAlert(alertName = a.event, alertString = a.description))
    )
  }
    ...
    (lat, lng) match {
      case (Valid(lat), Valid(lng)) =>
        for {
          ...
          owmresp <- client.onecall(lat, lng)
